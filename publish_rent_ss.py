...
def publish_ss_q(scraped, description, driver, info_for_database):
    ...
    def until_login():
        # LOGIN
        try:
            driver.execute_script("arguments[0].click();", wait_until_xpath(10,driver,'//*[@id="__next"]/header/div/div[2]/button[2]') )
        except:
        ...
        click('//*[@id="create-app-type"]/div[3]/div[2]')
...
#-----------------------sadzinebeli-------------------------------------------
    time.sleep(1)
    sadzinebeli_container = byxpath('//*[@id="create-app-details"]/div[2]/div[2]/div[2]/div[1]')
...
#-------------------------------aivani lojia-----------------------------------
    ...
    sul_raodenoba = scraped["balconies"] + loggia
...
#------------------------------sveli wertili------------------------------------
    ...
    if scraped["bathroom_type_id"] == None:
...
#------------------------------damatebiti informeishen------------------------------------
    ...
    if scraped["store_room_area"] != None:
        click('//*[@id="create-app-additional-info"]/div[2]/div[16]') # satavso
    # if scraped["sartuli"] == scraped["sartuli-sul"]:
    #     click('//*[@id="create-app-additional-info"]/div[2]/div[6]') #bolo sartuli
        #GARAJI DA SARDAPI AR GVAQVS
#--------------------------descripsheni----------------------------------------
...
#publish_ss_q(scraped, "ქირავდება პირველი და ბოლო თვის წინასწარი გადახდით!!!ქირავდება კეთილმოწყობილი ბინა ყველანაირი ავეჯითა და ტექნიკით რაც დაგჭირდებათ უზრუნველყოფილად ცხოვრებისთვის ბინა არის კარგ ადგილას ყველა საჭირო ობიექტით გარშემორტყმული")



def publish_ss_q_house(scraped, description, driver, info_for_database):
...
#-----------------------sadzinebeli-------------------------------------------
    ...
    if scraped["sadzinebeli"] != None or scraped["sadzinebeli"] != "None":
        click_element(
...
#--------------------------------სტატუსის tipi----------------------------------
    match scraped["status"]:
        case "ახალი აშენებული":
            click('//*[@id="create-app-details"]/div[2]/div[5]/div[2]/div/div[1]')
        case "მშენებარე":
            click('//*[@id="create-app-details"]/div[2]/div[5]/div[2]/div/div[2]')
        case "ძველი აშენებული":
            click('//*[@id="create-app-details"]/div[2]/div[5]/div[2]/div/div[3]')


    #-----------------garemontebuli

    click('//*[@id="create-app-details"]/div[2]/div[6]/div[2]/div/div[4]')
#------------------------------damatebiti informeishen------------------------------------

    for addition in scraped.get("additions", []):  # Loop through all additions
        match addition:
            case "elevator":
            ...
            case "furniture":
                click('//*[@id="create-app-additional-info"]/div[2]/div[8]')
            case "refrigerator":
                click('//*[@id="create-app-additional-info"]/div[2]/div[7]')
            case "telephone":
                click('//*[@id="create-app-additional-info"]/div[2]/div[18]')
            case "tv":
                click('//*[@id="create-app-additional-info"]/div[2]/div[19]')
                click('//*[@id="create-app-additional-info"]/div[2]/div[4]')


    if scraped["heating_type"] != None:
        click('//*[@id="create-app-additional-info"]/div[2]/div[11]')
    if scraped["hot_water_type"] == 6:  # cent gatboba
        click('//*[@id="create-app-additional-info"]/div[2]/div[15]')
    if scraped["balconies"] != None:
        click('//*[@id="create-app-additional-info"]/div[2]/div[2]')  # aivani
    if scraped["store_room_area"] != None:
        click('//*[@id="create-app-additional-info"]/div[2]/div[3]')  # satavso
        click('//*[@id="create-app-additional-info"]/div[2]/div[17]')
    if scraped["sartuli"] == scraped["sartuli-sul"]:
        click('//*[@id="create-app-additional-info"]/div[2]/div[6]')  # bolo sartuli
...
def publish_ss_q_com(scraped, description, driver, info_for_database):
...
#
# publish_ss_q_com(qiravdeba_scraped,"gela",cdriver(), {})